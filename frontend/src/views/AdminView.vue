<template>
  <div class="flex h-screen bg-[#020617] text-slate-300 overflow-hidden font-sans relative">
      ...
      <div class="w-60 flex flex-col h-full flex-shrink-0">
        ...
        <div class="p-3">
          <div class="bg-slate-900/80 rounded-lg p-3 border border-slate-800">
            ...
            <div class="mt-1.5">
              <span v-if="employee?.day_completed"
                class="text-[7px] px-1.5 py-0.5 rounded bg-red-500/10 text-red-500 border border-red-500/20 uppercase font-bold">Terminado</span>
              <span v-else-if="employee?.is_inside"
                class="text-[7px] px-1.5 py-0.5 rounded bg-blue-500/10 text-blue-500 border border-blue-500/20 uppercase font-bold">En
                Turno</span>
              <span v-else
                class="text-[7px] px-1.5 py-0.5 rounded bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 uppercase font-bold">Disponible</span>
            </div>
          </div>
        </div>
        ...
        <div class="p-3 border-t border-slate-800">
          <button @click="logout"
            class="w-full py-2 text-slate-500 hover:text-white font-bold text-[9px] uppercase flex items-center justify-center gap-2 transition-colors">
            Cerrar Sesión
          </button>
        </div>
      </div>
    ...
    <main class="flex-1 flex flex-col min-w-0 overflow-hidden">
      <header
        class="bg-[#0F172A] border-b border-slate-800 p-2.5 flex items-center justify-between lg:justify-start gap-4">
        <div class="flex items-center gap-3">
          ...
          <h2 v-if="!isSidebarOpen" class="text-[#FF8F00] text-sm font-black uppercase italic animate-fade-in">Woden
            Admin</h2>
        </div>
      ...
      <div class="flex-1 p-4 md:p-6 overflow-y-auto flex flex-col gap-6">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div class="flex gap-2 w-full md:w-auto">
            ...
            <button @click="fetchReport"
              class="p-1.5 bg-slate-900 border border-slate-800 rounded hover:bg-slate-800 transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-[#FF8F00]" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
        ...
        <div v-if="showTable" class="bg-slate-900/50 border border-slate-800 rounded-xl overflow-x-auto shadow-2xl">
          <table class="w-full text-left text-[10px] min-w-[500px]">
            <thead
              class="bg-slate-800/40 text-[#FF8F00] font-bold uppercase tracking-tighter border-b border-slate-800">
              <tr>
            ...
            <tbody class="divide-y divide-slate-800/30">
              <tr v-for="item in filteredReport" :key="item.id" class="hover:bg-slate-800/20 transition-colors">
                ...
                <td class="px-4 py-2.5 text-right">
                  <span
                    :class="item.estado === 'TARDE' ? 'text-red-400 bg-red-400/5' : 'text-emerald-400 bg-emerald-400/5'"
...
const {
  ...
  fetchReport, handleAttendance, logout, showTable
} = adminOdoo();
</script>
