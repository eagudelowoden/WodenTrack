<template>
  <div v-if="employee"
    ...
    <header class="w-full max-w-[450px] flex items-center justify-between px-2">
      ...
      <button @click="toggleTheme" class="theme-toggle active:scale-90 p-3 rounded-xl border"
    ...
    <footer class="w-full max-w-[450px] space-y-8">
      <div class="action-grid">
        ...
        <button @click="handleAttendance" :disabled="loading || employee.is_inside"
          class="action-card btn-entrada group disabled:opacity-20 active:scale-95">
          <div class="w-12 h-12 rounded-xl flex items-center justify-center mb-3 bg-white/10 shadow-sm transition-transform group-hover:scale-110">
        ...
        <button @click="handleAttendance" :disabled="loading || !employee.is_inside"
          class="action-card btn-salida group disabled:opacity-20 active:scale-95">
          <div class="w-12 h-12 rounded-xl flex items-center justify-center mb-3 bg-white/10 shadow-sm transition-transform group-hover:scale-110">
            <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5">
