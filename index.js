...
// --- LOGIN CON VALIDACIONES ROBUSTAS ---
app.post('/login', (req, res) => {
    ...
    // 2. VALIDACIÓN: Credenciales básicas (Simetría PIN/Password)
    if (usuario !== password) {
    ...
    // 3. CONEXIÓN CON ODOO: Autenticación de Sistema
    common.methodCall('authenticate', [odooConfig.db, odooConfig.username, odooConfig.password, {}], (err, uid) => {
        if (err) {
            ...
            return res.status(503).json({ 
                status: "error", 
                message: "Odoo no responde. Verifique su conexión de red." 
            });
        }
        ...
        if (!uid) {
            return res.status(500).json({ 
                status: "error", 
                message: "Error de autenticación en el servidor Odoo" 
            });
        }

        // 4. BUSCAR EMPLEADO POR PIN
        models.methodCall('execute_kw', [
            odooConfig.db, uid, odooConfig.password,
            'hr.employee', 'search_read',
            [[['pin', '=', usuario]]], 
            { fields: ['id', 'name', 'job_id', 'pin'], limit: 1 }
        ], (err, employees) => {
            if (err) {
                return res.status(500).json({ 
                    status: "error", 
                    message: "Error al consultar la base de datos de empleados" 
                });
            }
            ...
            const esAdmin = cargo.includes("GERENTE") || cargo.includes("COORDINADOR") || 
                            cargo.includes("JEFE") || cargo.includes("IT") || 
                            cargo.includes("DESARROLLADOR");

            // 5. VALIDACIÓN DE ESTADO: ¿Está adentro o ya terminó el día?
            models.methodCall('execute_kw', [
                odooConfig.db, uid, odooConfig.password,
        ...
        models.methodCall('execute_kw', [
            ...
            if (lastAttendance && lastAttendance.length > 0) {
                ...
                models.methodCall('execute_kw', [
                    ...
                    res.json({ status: "success", type: "out", message: "Salida registrada con éxito" });
                });
            } else {
                // REGISTRAR ENTRADA
                models.methodCall('execute_kw', [
                    ...
                    res.json({ status: "success", type: "in", message: "Entrada registrada con éxito" });
                });
            }
        });
    });
...
app.listen(PORT, '0.0.0.0', () => console.log(`🚀 Servidor WodenTrack en puerto ${PORT}`));